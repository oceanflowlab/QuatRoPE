...
def calc_scanrefer_score_asr(preds, config=None):
    ...
    val_scores = {
        '[asr] Acc@0.25': float(iou25_acc) / len(preds),
        '[asr] Acc@0.50': float(iou50_acc) / len(preds)
    }
...
def calc_scanrefer_score(preds, config=None):
    ...
    for i, output in enumerate(preds):
        ...
        instance_locs = instance_attrs[scene_id]["locs"]
        scannet_locs = scannet_attrs[scene_id]["locs"]
        #print("unique_multiple_lookup", scene_id, unique_multiple_lookup[scene_id])
        unique_multiple = unique_multiple_lookup[scene_id][str(obj_id)]
        #unique_multiple = 0 if np.random.random() < 0.5 else 1
...
def calc_scanrefer_location_score(preds, config=None):
    ...
    for i, output in enumerate(preds):
        ...
        instance_locs = instance_attrs[scene_id]["locs"]
        scannet_locs = scannet_attrs[scene_id]["locs"]
        print("unique_multiple_lookup", unique_multiple_lookup[scene_id])
        unique_multiple = unique_multiple_lookup[scene_id][str(obj_id)]
        if unique_multiple == 0:
            unique_all += 1
        else:
